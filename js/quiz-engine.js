/* ============================================================
   Learn with Ms. Thúy — Quiz Engine (split v5)
   ============================================================ */
async function initPostPage() {
  ...
  function updateSoundBtn() {
    ...
    soundBtn.setAttribute('aria-pressed', window.getSoundOn());
  }
  updateSoundBtn();
  soundBtn.onclick = () => {
  ...
  // ============================================================
  // 🔌  BRIDGE — Cho các file types/*.js dùng chung
  // ============================================================
  window.Quiz = {
    game: game,
  ...
  document.getElementById('modalOk').addEventListener('click', () => {
    const cb = window.getModalAction(); window.closeModal(); cb && cb();
  });
  document.getElementById('modal').addEventListener('click', e => {
    if (e.target.dataset.close !== undefined) window.closeModal();
  });
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape' && !document.getElementById('modal').hidden) window.closeModal();
  });
  ...
  function renderQuestion() {
    ...
    if (!typeDef) {
      stage.innerHTML = '<div class="q-card"><p>❌ Chưa hỗ trợ dạng: <b>' + esc(item._partType) + '</b></p></div>';
      return;
    }
    ...
    // Chỉ hiện q-text khi type không tự render (showsQText !== false)
      ...
      wrongs.forEach(q => {
          ...
          (ans.details || []).filter(d => !d.ok).forEach(d => {
            if (d._isPara) {
              ...
              art.innerHTML =
                ...
                '<p class="review-q para-review-text" style="line-height:1.9;margin-top:8px;">' + window.paraReviewHtml(mockItem, mockAns) + '</p>' +
                (d.vi ? '<p class="review-vi">🇻🇳 ' + esc(d.vi) + '</p>' : '') +
                (d.ex ? '<p class="review-ex">💡 ' + esc(d.ex) + '</p>' : '');
              list.appendChild(art);
            } else {
              list.appendChild(window.reviewCard(q._partName, d.q, d.chosen, d.right, d.vi, d.ex));
            }
          });
          return;
        }
        ...
        if (q._partType === 'type-in-para') {
          ...
          art.innerHTML =
            ...
            '<p class="review-q para-review-text" style="line-height:1.9">' + window.paraReviewHtml(q, ans) + '</p>' +
        ...
        const rightAns =
          ...
          (q.o ? q.o[q.a] : '—');
        list.appendChild(window.reviewCard(q._partName, q.q, ans.chosen, rightAns, q.vi, q.ex, q));
      });
    }
