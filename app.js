...
async function initPostPage(){ 
  ...
  // ===== ĐÁNH SỐ "CÂU" LŨY KẾ CHO TỪNG ITEM =====
  // Mỗi item (mcq/fill/type-in/... = 1 "câu"; listening/matching/type-in-para
  // = N "câu" tương ứng số điểm _points của nó) đều được gán vị trí lũy kế
  // để thanh tiến trình và bộ đếm "câu X / tổng" tính TỪNG câu con của
  // listening/reading/matching ngang bằng với 1 câu bình thường,
  // thay vì tính cả khối listening/matching chỉ là "1 câu".
  ...
  function updateProgressUI() { 
    ...
    // "Câu số mấy" tính theo ĐIỂM lũy kế, để mỗi câu con của listening/reading/
    // matching/type-in-para được đếm ngang bằng 1 câu bình thường, thay vì cả
    // khối chỉ tính là "1 câu" như trước. 
    const cumBefore = item ? (item._cumBefore || 0) : 0; 

    // 1. Thanh tiến trình (dựa trên số điểm đã đi qua / tổng điểm) 
    ...
    // 2. Số câu đang làm / tổng số (đều tính theo điểm — game.total) 
    ...
    document.getElementById('listenSubmit').addEventListener('click', function(){ 
      ...
      subs.forEach((sub, si) => { 
        ...
        if(sub.qtype === 'single'){ 
          ...
          // (đã push riêng ở đây nên KHÔNG push thêm lần nữa ở cuối vòng lặp — xem alreadyPushed) 
          details.push({  
        ...
        if(ok) correctCount++; 
        // Sửa lỗi: chỉ push detail 1 lần / sub. Riêng "type-in-para" đã tự push
        // chi tiết đầy đủ (kèm đoạn văn gốc) ở nhánh xử lý phía trên rồi, nên
        // ở đây chỉ push cho các dạng còn lại — tránh bị nhân đôi thẻ review.
        if(!alreadyPushed){ 
