/* ============================================================ 
   Learn with Ms. Thúy — Quiz Engine (Full v4.1 + Timer & Progress + Type-In-Para Fixed) 
   Hỗ trợ: mcq | fill | type-in | type-in-para | reading | matching | synonym | antonym | listening 
   Tính điểm: Thlg 10 · điểm mỗi câu/sub-question = 10 / tổng_số_đơn_vị_tính_điểm 
   ============================================================ */ 
...
async function initPostPage(){ 
    ...
    items: post.parts.flatMap(part => { 
      if (part.type === 'listening') { 
        ...
        return [{ 
          ...
          _points: subsCount, // Mỗi sub-question = 1 điểm 
        }]; 
      } 
      ...
      if (part.type === 'type-in-para') { 
        return (part.questions || []).map(q => { 
          ...
          return { 
            ...
            _points: parsed.blanks.length || 1, // Mỗi chỗ trống = 1 điểm 
          }; 
        }); 
      } 
  ...
  pointHint.textContent = '💯 Thang điểm 10 · ' + game.total + ' đơn vị tính điểm · mỗi đơn vị = ' + game.pointPerQ.toLocaleString('vi-VN') + ' điểm'; 
  ...
  document.getElementById('exitBtn').addEventListener('click', () => { 
    openModal('Thoát bài quiz?', 'Kết quả hiện tại sẽ không được lưu.', 'Thoát', () => { location.href = 'index.html'; }); 
  }); 
  ...
  function updateLiveScore(){ 
    ...
    // Math.round đảm bảo không hiện số thập phân lẻ như 5.00000001
    pill.textContent = `✓ ${Math.round(game.correct)}/${game.total} (${liveScore}đ)`; 
  ...
  function renderQuestion(){ 
    ...
    if (item._partType === 'listening') { 
      ...
      '</div>'; 
      if (item._image) { 
        html += '<div class="listening-img-wrap"><img class="listening-img" src="' + esc(item._image) + '" alt="Hình minh họa" loading="lazy"></div>'; 
      } 
    } 
  ...
  function renderBodyByType(item){ 
    switch(item._partType){ 
      case 'mcq': case 'synonym': case 'antonym': return renderMCQ(item); 
      case 'reading': return renderMCQ(item); 
      case 'fill': return renderFill(item); 
      case 'type-in': return renderTypeIn(item); 
      case 'type-in-para': return renderTypeInPara(item); 
      case 'matching': return renderMatching(item); 
      case 'listening': return renderListening(item); 
  ...
  function renderMCQ(item){ 
    ...
    let html = ''; 
    if(item._passage){ 
      html += '<div class="reading-passage">' + esc(item._passage).split('\n\n').map(p => '<p style="margin-bottom:14px">' + esc(p) + '</p>').join('') + '</div>'; 
    } 
    html += '<div class="options">' + shuffled.map((opt,i) => 
  ...
  function renderFill(item){ 
    const pool = (item && Array.isArray(item.words) && item.words.length) ? item.words : (item && Array.isArray(item.a) ? item.a : ['answer']); 
    const shuffledWords = shuffle(pool); 
    item._shuffledWords = shuffledWords; 
    return '<div class="fill-wrap">' + 
      '<div class="word-bank"><div class="word-bank-label">💎 Chọn từ trong khung:</div><div class="word-bank-chips">' + 
        shuffledWords.map((w,i) => '<button class="word-chip" data-word="' + esc(w) + '" data-i="' + i + '" type="button">' + esc(w) + '</button>').join('') + 
      '</div></div>' + 
      '<input class="fill-input" id="fillInput" type="text" autocomplete="off" placeholder="Từ em chọn sẽ hiện ở đây..." readonly>' + 
      '<div class="fill-actions"><button class="btn btn-ghost" id="fillClear" type="button">🗑️ Xóa chọn</button><button class="btn btn-primary" id="fillCheck" type="button">✓ Kiểm tra</button></div>' + 
      '<div class="fill-hint">💡 Bấm vào một từ ở khung trên, rồi bấm <b>Kiểm tra</b></div></div>'; 
  } 
  ...
  function renderTypeIn(item){ 
    ...
    html += '<div class="typein-wrap">' + 
      '<input class="fill-input typein-input" id="typeinInput" type="text" autocomplete="off" placeholder="✍️ Gõ đáp án của em vào đây...">' + 
      '<div class="fill-actions"><button class="btn btn-primary" id="typeinCheck" type="button">✓ Kiểm tra</button></div>' + 
      '<div class="fill-hint">💡 Gõ từ / cụm từ em cho là đúng, rồi bấm <b>Kiểm tra</b> hoặc nhấn <b>Enter</b></div></div>'; 
    return html; 
  } 
  ...
  function renderTypeInPara(item){ 
    return '<div class="para-fill-wrap">' + 
      ...
      '<div class="fill-actions"><button class="btn btn-primary" id="paraCheck" type="button">✅ Nộp đoạn văn</button></div>' + 
      '<div class="fill-hint">💡 Điền từ thích hợp vào từng ô có đánh số, rồi bấm <b>Nộp đoạn văn</b> (nhấn <b>Enter</b> để qua ô tiếp theo)</div></div>'; 
  } 
  ...
  function renderMatching(item){ 
    ...
    item._left = left; item._right = right; item._matches = 0; item._completed = false; 
    return '<div class="matching-wrap">' + 
      ...
      '<div class="matching-grid">' + 
        '<div class="matching-col"><div class="matching-col-head">🔤 English</div>' + left.map(x => '<div class="match-item" data-side="L" data-i="' + x.idx + '">' + esc(x.text) + '</div>').join('') + '</div>' + 
        '<div class="matching-col"><div class="matching-col-head">🇻🇳 Tiếng Việt</div>' + right.map(x => '<div class="match-item" data-side="R" data-i="' + x.idx + '">' + esc(x.text) + '</div>').join('') + '</div>' + 
      '</div></div>'; 
  } 

  function subTypeLabel(t){ 
    return { single:'Chọn 1 đáp án', multi:'Chọn nhiều đáp án', fill:'Điền từ (có sẵn)', 'type-in':'Gõ đáp án', 'type-in-para':'Điền đoạn văn', match:'Ghép (dropdown)' }[t] || t; 
  } 

  function renderListening(item){ 
    const LETTERS = ['A','B','C','D','E','F','G','H']; 
    (item._subs || []).forEach(sub => { 
      if(sub.qtype === 'single'){ sub._shuffled = shuffle((sub.o || []).map((t,i) => ({text:t, correct:i === sub.a}))); } 
      else if(sub.qtype === 'multi'){ sub._shuffled = shuffle((sub.o || []).map((t,i) => ({text:t, correct:(sub.a || []).includes(i)}))); } 
      else if(sub.qtype === 'fill'){ if(sub.words && sub.words.length) sub._shuffledWords = shuffle(sub.words); } 
    ...
    (item._subs || []).forEach((sub, si) => { 
      ...
      if(sub.qtype === 'type-in-para'){ 
        html += '<div class="para-fill-wrap para-fill-sub"><p class="para-fill-text" style="line-height:2;font-size:1.02rem">' + sub._parsedHtml + '</p></div>'; 
      } else {
        html += '<p class="sub-text">' + esc(sub.q || '').split('____').join('<span class="blank">&nbsp;</span>') + '</p>'; 
      }

      if(sub.qtype === 'single'){ 
        html += '<div class="options">' + sub._shuffled.map((opt,i) => '<button class="opt sub-opt" data-sub="' + si + '" data-i="' + i + '" type="button"><span class="opt-letter">' + LETTERS[i] + '</span><span class="opt-text">' + esc(opt.text) + '</span></button>').join('') + '</div>'; 
      } else if(sub.qtype === 'multi'){ 
        html += '<div class="options">' + sub._shuffled.map((opt,i) => '<button class="opt sub-multi" data-sub="' + si + '" data-i="' + i + '" type="button"><span class="opt-letter">' + LETTERS[i] + '</span><span class="opt-text">' + esc(opt.text) + '</span><span class="multi-check" aria-hidden="true">✓</span></button>').join('') + '</div>'; 
      } else if(sub.qtype === 'fill'){ 
        if(sub._shuffledWords){ html += '<div class="word-bank"><div class="word-bank-label">💎 Chọn từ:</div><div class="word-bank-chips">' + sub._shuffledWords.map(w => '<button class="word-chip sub-chip" data-sub="' + si + '" data-word="' + esc(w) + '" type="button">' + esc(w) + '</button>').join('') + '</div></div>'; } 
        html += '<input class="fill-input sub-fill-input" data-sub="' + si + '" type="text" autocomplete="off" placeholder="' + (sub._shuffledWords ? 'Từ đã chọn...' : 'Gõ từ em nghe được...') + '"' + (sub._shuffledWords ? ' readonly' : '') + '>'; 
      } else if(sub.qtype === 'type-in'){ 
        html += '<input class="fill-input sub-fill-input sub-typein-input" data-sub="' + si + '" type="text" autocomplete="off" placeholder="✍️ Gõ đáp án em nghe được...">'; 
      } else if(sub.qtype === 'match'){ 
        html += '<div class="match-opts"><b>📋 Phương án:</b><br>' + (sub.options || []).map((op,i) => '<b>' + LETTERS[i] + '.</b> ' + esc(op) + '<br>').join('') + '</div><div class="match-rows">'; 
        (sub.rows || []).forEach((row, ri) => { html += '<div class="match-row"><span class="row-label">' + esc(row.left) + '</span><select data-row="' + ri + '"><option value="">— ? —</option>' + (sub.options || []).map((op,i) => '<option value="' + i + '">' + LETTERS[i] + '</option>').join('') + '</select></div>'; }); 
        html += '</div>'; 
      } 
      html += '</div>'; 
    }); 
    html += '</div><button class="btn btn-primary listen-submit" id="listenSubmit" type="button">✅ Nộp bài nghe</button>'; 
    return html; 
  } 

  function attachEventsByType(item){ 
    switch(item._partType){ 
      case 'mcq': case 'synonym': case 'antonym': case 'reading': 
        document.querySelectorAll('#qStage .opt').forEach(btn => btn.addEventListener('click', () => handleMCQ(btn, item))); break; 
  ...
  function handleMCQ(btn, item){ 
    ...
    if(chosen.correct){ 
      btn.classList.add('correct'); all.forEach((b,j) => { if(j !== i) b.classList.add('dim'); }); 
      game.correct++; updateLiveScore(); sfxCorrect(); 
      showToast('✨ Chính xác! + ' + game.pointPerQ.toLocaleString('vi-VN') + ' điểm', 'good'); 
      setTimeout(nextStep, 1000); 
    } else { 
      btn.classList.add('wrong'); all.forEach((b,j) => { if(j !== i) b.classList.add('dim'); }); 
      sfxWrong(); showToast('❌ Chưa đúng…', 'bad'); setTimeout(nextStep, 1500); 
    } 
  } 
  ...
  function handleFill(item){ 
    ...
    chips.forEach(chip => { 
      chip.addEventListener('click', () => { 
        ...
        input.value = chip.dataset.word; input.classList.remove('correct','wrong'); sfxTick(); 
      }); 
    }); 
    clear.addEventListener('click', () => { 
      if(done) return; input.value = ''; input.classList.remove('correct','wrong'); 
    ...
    function doCheck(){ 
      ...
      if(!val){ showToast('💡 Em hãy chọn một từ trong khung trước nhé!', ''); return; } 
      done = true; chips.forEach(c => { c.disabled = true; if(c !== selectedChip) c.classList.add('dim'); }); 
      clear.disabled = true; check.disabled = true; 
      const ok = item.a.some(ans => val === ans.toLowerCase()); 
      game.answers[game.idx] = {chosen: input.value.trim(), ok}; 
      if(ok){ 
        input.classList.add('correct'); if(selectedChip) selectedChip.classList.add('chip-correct'); 
        game.correct++; updateLiveScore(); sfxCorrect(); 
        showToast('✨ Chính xác! + ' + game.pointPerQ.toLocaleString('vi-VN') + ' điểm', 'good'); setTimeout(nextStep, 1200); 
      } else { 
        input.classList.add('wrong'); if(selectedChip) selectedChip.classList.add('chip-wrong'); sfxWrong(); 
        const correctWord = item.a[0].toLowerCase(); 
        chips.forEach(c => { if(c.dataset.word.toLowerCase() === correctWord){ c.classList.add('chip-correct'); c.classList.remove('dim'); } }); 
        const show = document.createElement('div'); show.className = 'fill-show'; show.innerHTML = '✅ Đáp án đúng: <b>' + esc(item.a[0]) + '</b>'; 
        document.querySelector('.fill-wrap').appendChild(show); showToast('❌ Chưa đúng rồi…', 'bad'); setTimeout(nextStep, 2200); 
      } 
    } 
  ...
  function handleTypeIn(item){ 
    ...
    function doCheck(){ 
      ...
      if(!val){ showToast('💡 Em hãy gõ đáp án trước nhé!', ''); input.focus(); return; } 
      ...
      if(ok){ 
        input.classList.add('correct'); game.correct++; updateLiveScore(); sfxCorrect(); 
        showToast('✨ Chính xác! + ' + game.pointPerQ.toLocaleString('vi-VN') + ' điểm', 'good'); setTimeout(nextStep, 1200); 
      } else { 
        input.classList.add('wrong'); sfxWrong(); 
        const show = document.createElement('div'); show.className = 'fill-show'; show.innerHTML = '✅ Đáp án đúng: <b>' + esc(answers[0]) + '</b>'; 
        document.querySelector('.typein-wrap').appendChild(show); showToast('❌ Chưa đúng rồi…', 'bad'); setTimeout(nextStep, 2200); 
      } 
    } 
    input.addEventListener('keydown', e => { if(e.key === 'Enter') doCheck(); }); 
    check.addEventListener('click', doCheck); 
  ...
  function handleTypeInPara(item){ 
    ...
    function doCheck(){ 
      ...
      inputs.forEach(inp => inp.disabled = true); btn.disabled = true; 
      let correctCount = 0; const chosenParts = [], rightParts = []; 
      ...
      inputs.forEach((inp, i) => { 
        ...
        chosenParts.push(val || '(bỏ trống)'); rightParts.push(blank.answers[0]); 
        if(!ok){ 
          const badge = inp.closest('.para-blank-wrap'); 
          if(badge){ 
            const show = document.createElement('span'); show.className = 'para-blank-correct'; 
            show.style.cssText = 'color:#2ea36c;font-size:.85em;font-weight:700;margin-left:2px'; 
            show.textContent = '✅ ' + blank.answers[0]; badge.appendChild(show); 
          } 
        } 
      }); 

      game.correct += correctCount; updateLiveScore(); 
      game.answers[game.idx] = { chosen: chosenParts.join(' | '), right: rightParts.join(' | '), ok: correctCount === inputs.length }; 

      if(correctCount === inputs.length){ sfxCorrect(); showToast('✨ Điền đúng hết ' + inputs.length + ' chỗ trống!', 'good'); } 
      else if(correctCount > 0){ sfxTick(); showToast('📝 Đúng ' + correctCount + '/' + inputs.length + ' chỗ trống', ''); } 
      else { sfxWrong(); showToast('❌ Chưa đúng chỗ nào…', 'bad'); } 
      setTimeout(nextStep, 2200); 
    } 

    btn.addEventListener('click', doCheck); 
    inputs.forEach((inp, i) => { 
      inp.addEventListener('keydown', e => { if(e.key === 'Enter'){ e.preventDefault(); const next = inputs[i+1]; if(next) next.focus(); else doCheck(); } }); 
    }); 
  ...
  function handleMatching(item){ 
    ...
    document.querySelectorAll('#qStage .match-item').forEach(el => { 
      el.addEventListener('click', () => { 
        if(item._completed || el.classList.contains('matched')) return; 
        if(el.dataset.side === 'L'){ if(selL) selL.classList.remove('selected'); selL = el; el.classList.add('selected'); } 
        else { if(selR) selR.classList.remove('selected'); selR = el; el.classList.add('selected'); } 
        sfxTick(); 
        if(selL && selR){ 
          const li = +selL.dataset.i, ri = +selR.dataset.i; 
          if(li === ri){ 
            selL.classList.remove('selected'); selR.classList.remove('selected'); 
            selL.classList.add('matched'); selR.classList.add('matched'); sfxCorrect(); 
            item._matches++; game.correct++; updateLiveScore(); 
            const countEl = document.getElementById('matchCount'); if(countEl) countEl.textContent = item._matches; 
            if(item._matches === item._pairs.length){ 
              item._completed = true; game.answers[game.idx] = {chosen: 'matched all', ok: true}; 
              showToast('🎉 Ghép hết rồi!', 'good'); 
              const statusEl = document.querySelector('#qStage .matching-wrap span:last-child'); if(statusEl) statusEl.textContent = '✅ Hoàn thành'; 
              document.querySelectorAll('#qStage .match-item:not(.matched)').forEach(el => { el.style.opacity = '0.5'; el.style.cursor = 'default'; }); 
              setTimeout(nextStep, 1000); 
            } 
          } else { 
            selL.classList.add('wrong-pair'); selR.classList.add('wrong-pair'); sfxWrong(); 
            const a = selL, b = selR; setTimeout(() => { a.classList.remove('wrong-pair','selected'); b.classList.remove('wrong-pair','selected'); }, 500); 
          } 
          selL = null; selR = null; 
        } 
      }); 
  ...
  function handleListening(item){ 
    ...
    stage.querySelectorAll('.sub-opt').forEach(btn => { 
      btn.addEventListener('click', () => { 
        ...
        btn.classList.add('selected'); sfxTick(); 
      }); 
    }); 
    stage.querySelectorAll('.sub-multi').forEach(btn => { btn.addEventListener('click', () => { btn.classList.toggle('picked'); sfxTick(); }); }); 
    ...
    document.getElementById('listenSubmit').addEventListener('click', function(){ 
      const submit = this; if(submit.disabled) return; 
      let correctCount = 0; const details = []; 

      subs.forEach((sub, si) => { 
        const wrap = stage.querySelector('.sub-q[data-sub="'+si+'"]'); 
        let ok = false, chosenText = '—', rightText = '—'; 
        let subDetailsExtra = {}; // FIX: Gom dữ liệu extra vào đây để push 1 lần duy nhất

        if(sub.qtype === 'single'){ 
          const sel = wrap.querySelector('.sub-opt.selected'); const selI = sel ? +sel.dataset.i : -1; 
          chosenText = selI >= 0 ? sub._shuffled[selI].text : '(không chọn)'; rightText = sub.o[sub.a]; 
          ok = selI >= 0 && sub._shuffled[selI].correct; 
          wrap.querySelectorAll('.sub-opt').forEach((b,bi) => { b.disabled = true; if(sub._shuffled[bi].correct) b.classList.add('correct'); else if(bi === selI) b.classList.add('wrong'); else b.classList.add('dim'); }); 
        } 
        else if(sub.qtype === 'multi'){ 
          ...
          wrap.querySelectorAll('.sub-multi').forEach((b,bi) => { b.disabled = true; if(sub._shuffled[bi].correct) b.classList.add('correct'); else if(picked.includes(bi)) b.classList.add('wrong'); else b.classList.add('dim'); }); 
        } 
        else if(sub.qtype === 'fill'){ 
          const input = wrap.querySelector('.sub-fill-input'); const val = (input.value || '').trim().toLowerCase(); 
          chosenText = input.value.trim() || '(không điền)'; rightText = sub.a[0]; 
          ok = sub.a.some(ans => val === ans.toLowerCase()); input.disabled = true; input.classList.add(ok ? 'correct' : 'wrong'); 
          wrap.querySelectorAll('.sub-chip').forEach(c => c.disabled = true); 
          if(!ok){ const show = document.createElement('div'); show.className = 'fill-show'; show.innerHTML = '✅ Đáp án: <b>' + esc(sub.a[0]) + '</b>'; wrap.appendChild(show); } 
        } 
        else if(sub.qtype === 'type-in'){ 
          const input = wrap.querySelector('.sub-fill-input'); const val = (input.value || '').trim(); 
          chosenText = val || '(không điền)'; const answers = Array.isArray(sub.a) ? sub.a : [sub.a]; rightText = answers[0] || '—'; 
          ...
          if(!ok){ const show = document.createElement('div'); show.className = 'fill-show'; show.innerHTML = '✅ Đáp án: <b>' + esc(rightText) + '</b>'; wrap.appendChild(show); } 
        } 
        else if(sub.qtype === 'type-in-para'){ 
          const inputs = [...wrap.querySelectorAll('.para-blank-input')]; 
          let allOk = true; const chosenParts = [], rightParts = []; 
          inputs.forEach((inp, bi) => { 
            ...
            inp.disabled = true; inp.classList.add(isRight ? 'correct' : 'wrong'); 
            chosenParts.push(val || '(bỏ trống)'); rightParts.push(blank.answers[0]); 
            if(!isRight){ 
              const badge = inp.closest('.para-blank-wrap'); 
              if(badge){ const show = document.createElement('span'); show.className = 'para-blank-correct'; show.style.cssText = 'color:#2ea36c;font-size:.85em;font-weight:700;margin-left:2px'; show.textContent = '✅ ' + blank.answers[0]; badge.appendChild(show); } 
            } 
          }); 
          ok = allOk; chosenText = chosenParts.join(' | '); rightText = rightParts.join(' | '); 
          // FIX: Gán vào biến tạm, không push trực tiếp vào details ở đây
          subDetailsExtra = { _isPara: true, _originalText: sub.text || sub.passage || sub.q, _blanks: sub._blanks }; 
        } 
        else if(sub.qtype === 'match'){ 
          const rows = sub.rows || []; const picks = []; let rowOk = true; 
          rows.forEach((row, ri) => { 
            const sel = wrap.querySelector('select[data-row="'+ri+'"]'); const v = sel.value === '' ? -1 : +sel.value; 
            sel.disabled = true; const good = v === row.a; if(!good) rowOk = false; 
            sel.classList.add(good ? 'sel-correct' : 'sel-wrong'); picks.push(v >= 0 ? LETTERS[v] : '—'); 
            if(!good){ const hint = document.createElement('span'); hint.className = 'row-hint'; hint.textContent = '✅ ' + LETTERS[row.a]; sel.parentNode.appendChild(hint); } 
          }); 
          ok = rowOk; chosenText = picks.join(', '); rightText = rows.map(r => LETTERS[r.a]).join(', '); 
        } 

        if(ok) correctCount++; 
        
        // FIX: PUSH MỘT LẦN DUY NHẤT CHO TẤT CẢ CÁC LOẠI, kèm theo dữ liệu extra nếu có
        details.push({ 
          q: sub.qtype === 'type-in-para' ? `Đoạn văn điền từ (Câu ${si+1})` : (sub.q || ''), 
          chosen: chosenText, right: rightText, ok: ok, vi: sub.vi, ex: sub.ex, 
          ...subDetailsExtra 
        }); 
      }); 

      game.correct += correctCount; updateLiveScore(); 
      game.answers[game.idx] = { chosen: correctCount + '/' + subs.length, ok: correctCount === subs.length, details }; 

      submit.disabled = true; submit.textContent = '✅ Đã nộp · Đúng ' + correctCount + '/' + subs.length; 
  ...
  function nextStep(){ 
    const au = document.getElementById('listenAudio'); if(au) au.pause(); 
    ...
    setTimeout(() => { game.idx++; renderQuestion(); updateProgressUI(); }, 220); 
  } 
  ...
  function finish(isForced = false){ 
    ...
    let title, msg; 
    if (isForced) { title = '⏰ Hết giờ, <span>' + esc(name) + '</span>!'; msg = 'Bài làm đã được tự động nộp. Em xem lại kết quả và rút kinh nghiệm nhé.'; } 
    else if(finalScore >= 9.5){ title='🏆 Điểm tuyệt đối, <span>'+esc(name)+'</span>!'; msg='Cô Thúy cực kỳ tự hào về em!'; } 
    ...
    wrap.innerHTML = 
      '<div class="result-head"><span class="eyebrow">📊 Kết quả bài làm</span><h1>' + title + '</h1><p class="result-msg">' + esc(msg) + '</p></div>' + 
      '<div class="score-board"><div class="ring-box"><svg viewBox="0 0 160 160"><defs><linearGradient id="ringGrad" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="#7a67ee"/><stop offset="55%" stop-color="#5a9df5"/><stop offset="100%" stop-color="#3ecfa0"/></linearGradient></defs><circle class="ring-bg" cx="80" cy="80" r="70"/><circle class="ring-fg" id="ringFg" cx="80" cy="80" r="70"/></svg><div class="ring-center"><div class="ring-pct" id="pctText">0%</div><div class="ring-label">chính xác</div></div></div>' + 
      '<div class="score-details"><div class="score-big"><b style="font-size:3rem;background:var(--grad-main);-webkit-background-clip:text;background-clip:text;color:transparent">' + finalScore.toFixed(1) + '</b><span style="font-size:1.4rem">/ 10 điểm</span></div>' + 
      '<div class="mini-stats"><div class="mini-stat"><b>' + Math.round(correct) + '/' + total + '</b><span>câu đúng</span></div><div class="mini-stat"><b>' + pointPerQ.toLocaleString('vi-VN') + '</b><span>điểm / đơn vị</span></div><div class="mini-stat"><b>' + mins + ':' + String(secs).padStart(2,'0') + '</b><span>thời gian</span></div><div class="mini-stat"><b>' + wrongs.length + '</b><span>cần ôn</span></div></div></div></div>' + 
      '<div class="result-actions"><a href="post.html?id=' + encodeURIComponent(post.id) + '" class="btn btn-primary">🔁 Làm lại</a><a href="index.html" class="btn btn-ghost">🏠 Về trang chủ</a><button class="btn btn-ghost" id="reviewBtn">📖 Xem câu sai</button></div>' + 
      '<div class="review"><h2>📝 Câu chưa đúng (' + wrongs.length + ')</h2><p class="review-sub">Đọc kỹ nghĩa tiếng Việt và giải thích nhé.</p><div id="reviewList"></div></div>'; 

    showScreen('result'); 

    const C = 439.82; const ring = document.getElementById('ringFg'); ring.style.strokeDashoffset = C; 
    setTimeout(() => ring.style.strokeDashoffset = C * (1 - pct/100), 200); 
    const t0 = performance.now(); 
    (function count(now){ const k = Math.min(1,(now-t0)/1200); const e = 1 - Math.pow(1-k,3); document.getElementById('pctText').textContent = Math.round(pct*e) + '%'; if(k<1) requestAnimationFrame(count); })(t0); 

    const list = document.getElementById('reviewList'); 
    if(!wrongs.length){ 
      list.innerHTML = '<div class="review-perfect">🎉 Tuyệt đối! Em làm đúng hết ' + total + ' đơn vị · đạt ' + finalScore.toFixed(1) + '/10 điểm! 🎉</div>'; 
    } else { 
      wrongs.forEach(q => { 
        ...
        if(q._partType === 'listening'){ 
          (ans.details || []).filter(d => !d.ok).forEach(d => { 
            if (d._isPara) { 
              const art = document.createElement('article'); art.className = 'review-item'; 
              const mockItem = { text: d._originalText, _blanks: d._blanks }; 
              const mockAns = { chosen: d.chosen }; 
              art.innerHTML = '<div class="review-head"><span class="badge-wrong">Chưa đúng</span><span style="font-family:Space Mono,monospace;font-size:.75rem;color:var(--ink-faint)">' + esc(q._partName || '') + ' - ' + esc(d.q) + '</span></div>' + 
                '<p class="review-q para-review-text" style="line-height:1.9; margin-top:8px;">' + paraReviewHtml(mockItem, mockAns) + '</p>' + 
                (d.vi ? '<p class="review-vi">🇻🇳 ' + esc(d.vi) + '</p>' : '') + (d.ex ? '<p class="review-ex">💡 ' + esc(d.ex) + '</p>' : ''); 
        ...
        if(q._partType === 'type-in-para'){ 
          const art = document.createElement('article'); art.className = 'review-item'; 
          art.innerHTML = '<div class="review-head"><span class="badge-wrong">Chưa đúng</span><span style="font-family:Space Mono,monospace;font-size:.75rem;color:var(--ink-faint)">' + esc(q._partName || '') + '</span></div>' + 
            '<p class="review-q para-review-text" style="line-height:1.9">' + paraReviewHtml(q, ans) + '</p>' + 
            (q.vi ? '<p class="review-vi">🇻🇳 ' + esc(q.vi) + '</p>' : '') + (q.ex ? '<p class="review-ex">💡 ' + esc(q.ex) + '</p>' : ''); 
          list.appendChild(art); 
          return; 
        } 

        const answersArr = Array.isArray(q.a) ? q.a : [q.a]; 
        const rightAns = (q._partType === 'fill' || q._partType === 'type-in') ? answersArr[0] : (q._partType === 'matching') ? 'matched all' : (q.o ? q.o[q.a] : '—'); 
        list.appendChild(reviewCard(q._partName, q.q, ans.chosen, rightAns, q.vi, q.ex, q)); 
      }); 
    } 

    document.getElementById('reviewBtn').onclick = () => document.querySelector('.review').scrollIntoView({behavior:'smooth'}); 
    if(finalScore >= 8 && !isForced){ setTimeout(launchConfetti, 350); setTimeout(sfxFanfare, 300); } 
  } 
}
